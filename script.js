...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Host
  if (btnCreateGame) btnCreateGame.addEventListener('click', createGameAsHost);
  if (btnJoinGameHost) btnJoinGameHost.addEventListener('click', joinGameAsHost);
  if (btnStartGame) btnStartGame.addEventListener('click', startGameHost);
  if (btnDrawCard) btnDrawCard.addEventListener('click', drawCardHost);
  if (btnEndGame) btnEndGame.addEventListener('click', confirmEndGame);
  if (btnHostSendChat) btnHostSendChat.addEventListener('click', sendChatAsHost);
  if (btnEndFingerPower) btnEndFingerPower.addEventListener('click', finalizeFingerPower);

  // Player
  if (btnEnterCodePlayer) btnEnterCodePlayer.addEventListener('click', enterCodePlayer);
  if (btnJoinPlayerGame) btnJoinPlayerGame.addEventListener('click', registerPlayerInGame);
  if (btnDrawCardPlayer) btnDrawCardPlayer.addEventListener('click', drawCardPlayer);
  if (btnUseJoker) btnUseJoker.addEventListener('click', useJokerPlayer);
  if (btnActivateFinger) btnActivateFinger.addEventListener('click', activateFingerPower);
  if (btnFingerClick) btnFingerClick.addEventListener('click', fingerClick);
  if (btnPlayerSendChat) btnPlayerSendChat.addEventListener('click', sendChatAsPlayer);

  // Câmera
  if (btnTakePhoto) btnTakePhoto.addEventListener('click', startCamera);
  if (btnCapture) btnCapture.addEventListener('click', capturePhoto);
  if (btnRetake) btnRetake.addEventListener('click', startCamera);
  if (btnAcceptPhoto) btnAcceptPhoto.addEventListener('click', acceptPhoto);
  if (btnCancelPhoto) btnCancelPhoto.addEventListener('click', closeCamera);
  ...
  // Verificar se há código de jogo na URL
  checkUrlForGameId();
  
  // Verificar se há jogo salvo no localStorage
  checkLocalStoragePlayer();
});

/********** VERIFICAÇÃO INICIAL E RECONNECT **********/
// Verifica se tem um código de jogo na URL
